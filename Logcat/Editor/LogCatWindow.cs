...
public class LogCatWindow : EditorWindow
{
    ...
    private string igornByString = String.Empty;
    string[] igorns = new string[0];
    ...
    private bool IsNoIgorn(string v)
    {
        for (int i = 0; i < igorns.Length; i++)
        {
            var s = igorns[i];
            if (s.Length > 0 && v.Contains(s))
            {
                return false;
            }
        }
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("保存", GUILayout.Width(60)))
        {
            lock (logsList)
            {
                ...
                {
                    lines.Add(logsList[i].BaseMessage);
    
                }
                System.IO.File.WriteAllLines("D:\\logcat.txt", lines);
                lines.Clear();
                for (int i = 0; i < filteredList.Count; i++)
                {
                    lines.Add(filteredList[i].BaseMessage);

                }
                System.IO.File.WriteAllLines("D:\\logcatFiltered.txt", lines);
            }
        }
        //logsList
        ...
        igornByString = GUILayout.TextField(igornByString, GUILayout.Height(20));
        igorns = igornByString.Split('|');
        for (int i = 0; i < igorns.Length; i++)
        {
            igorns[i] = igorns[i].Trim();
        }

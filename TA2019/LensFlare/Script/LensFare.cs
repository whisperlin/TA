...
public class LensFare : MonoBehaviour {
    ...
    if (vis)
      ...
      projPos.y = -projPos.y;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                cols[i0++] = new Color(ids.w, ids.w, ids.w)* color;
                cols[i0++] = new Color(ids.w, ids.w, ids.w)* color;
                cols[i0++] = new Color(ids.w, ids.w, ids.w)* color;
                cols[i0++] = new Color(ids.w, ids.w, ids.w)* color;
    ...
    private void OnEnable()
    {
        ...
        if (null != mOccludieCamera)
        {
            mOccludieCamera.gameObject.SetActive(true);
        }
    }
    private void OnDisable()
    {
        ...
        if (null != mOccludieCamera)
        {
            mOccludieCamera.gameObject.SetActive(false);
        }
    }
    private void OnDestroy()
    {
        if (null != sunObj)
        {
            GameObject.DestroyImmediate(sunObj);
        }
        if (null != mOccludieCamera)
        {
            GameObject.DestroyImmediate(mOccludieCamera.gameObject);
        }
    }
}

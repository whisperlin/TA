...
#if !defined(LIGHTMAP_OFF) || defined(LIGHTMAP_ON) 
	#define SHADOW_UVS(id0) float2 uv1 : TEXCOORD##id0;
#else
	#define SHADOW_UVS(id0) SHADOW_COORDS(id0)
	/*#if UNITY_SHADOW
...
#else
	#if UNITY_SHADOW
		#define VS_FILL_SHADOW_DATA(o,uv) TRANSFER_SHADOW(o);
	#else
		#define VS_FILL_SHADOW_DATA(o,uv) o._ShadowCoord = 0;
	#endif
#endif
...
#if !defined(LIGHTMAP_OFF) || defined(LIGHTMAP_ON)
	#define LIGHT_MAP_FINAL(i)\
	float attenuation = UNITY_SAMPLE_TEX2D(unity_ShadowMask, i.uv1).r;\
	float3 lightmap = DecodeLightmap(UNITY_SAMPLE_TEX2D(unity_Lightmap, i.uv1));
	//float attenuation = UNITY_SAMPLE_TEX2D(unity_ShadowMask, i.uv1).r;

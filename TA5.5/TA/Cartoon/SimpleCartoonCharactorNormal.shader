...
Shader "TA/SimpleCartoonCharactor Normal"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float4 _Normal_var = tex2D(_Normal,TRANSFORM_TEX(i.uv, _Normal));
				float3 normalLocal = _Normal_var.rgb;
				float3 normalDirection = normalize(mul(normalLocal, tangentTransform)); // Perturbed normals

				half ndl = dot(normalDirection,  VirtualDirectLight0.rgb );

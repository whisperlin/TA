Shader "YuLongZhi/BumpedSpecular EX"
{
	Properties
	{
	...
	sampler2D _AlphaCtrl;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				c.rgb = lerp(c.rgb, _SnowColor.rgb, nt *_SnowColor.a);
#endif
#endif
#if _ISWEATHER_ON
#if RAIN_ENABLE 
...
/*#if RAIN_ENABLE || SNOW_ENABLE
				...
				fixed3 diffuse = ambient + lightColor *  (nl*(1- _DifSC) + _DifSC)   * c.rgb + saturate(-nl0)*_BackColor.rgb*sss_scatter0* c.rgb;
				...
				diffuse *= lm;
				float shadow0 =  min( 1,dot(lm,half3(0.3,0.6,0.1))) ;
				//return float4(shadow0.rrr, 1);
				shadow0 = shadow0* shadow0*shadow0;
				spec *= shadow0;
				//return float4(shadow0.rrr, 1);
#endif
				c.rgb = diffuse + spec;

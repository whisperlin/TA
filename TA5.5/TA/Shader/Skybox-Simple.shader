Shader "TA/Skybox/Skybox Sun Clound" {
	Properties{
		...
		_CloundTex("Texture", 2D) = "black" {}
		_CloundSpeed("云层速度",Range(-1,1)) = 0.01

		[Toggle(_CLOUND2)] _CLOUND2("云2", Float) = 1
		_ClondColor2("云层色2",Color) = (1,1,1,1)
		_CloundTex2("Texture2", 2D) = "black" {}
		_CloundSpeed2("云层速度2",Range(-1,1)) = 0.01

		[Toggle(_SKY_FOG)] _SKY_FOG("雾", Float) = 1
		_SkyFogColor("雾色",Color) = (1,1,1,1)
		_SkyFogCtrl("雾调节",Range(0.01,1)) = 0.2
		...
		SubShader{
			...
			Pass {
				...
				#pragma   multi_compile  _SKY_FOG _
				...
				sampler2D _CloundTex2;
				float4 _CloundTex2_ST;
				float4 _ClondColor2;
				float _CloundSpeed2;
		...
		#endif
		#if _CLOUND2
					_CloundTex2_ST.z += _Time.x*_CloundSpeed2;
					fixed4 col2 = tex2D(_CloundTex2, i.uv.xy*_CloundTex2_ST.xy + _CloundTex2_ST.zw);
					cout.rgb = lerp(cout.rgb, _ClondColor2.rgb*col2.rgb, col2.a*_ClondColor2.a);
		#endif

		#if _CLOUND
					_CloundTex_ST.z += _Time.x*_CloundSpeed;
					fixed4 col = tex2D(_CloundTex,  i.uv.xy*_CloundTex_ST.xy + _CloundTex_ST.zw);
					cout.rgb = lerp(cout.rgb, _ClondColor.rgb , col.a*_ClondColor.a);
		#endif
...
#endif
					float fogFactor =   smoothstep0(  _SkyFogCtrl, viewDirection.y)  ;

#if _SKY_FOG
					cout.rgb = lerp(_SkyFogColor.rgb,cout.rgb, fogFactor);
#endif
					//cout.rgb = 
					return cout;
